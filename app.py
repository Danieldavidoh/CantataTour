    ...
    if os.path.exists(file_path):
        if file_type.startswith('image/'):
            ...
            if base64_data:
                ...
                try:
                    with open(file_path, "rb") as f:
                        st.download_button(label=f"⬇️ {file_name} 다운로드 (인라인 실패)", data=f.read(), file_name=file_name, mime=file_type, key=f"{key_prefix}_download_{notice_id}_{file_name}_imgfallback")
                except Exception:
                    pass
        elif file_type.startswith('video/'):
            try:
                with open(file_path, "rb") as f:
                    st.video(f.read(), format=file_type, start_time=0)
            ...
            try:
                with open(file_path, "rb") as f:
                    st.download_button(label=f"⬇️ 📄 {file_name} ({file_size_kb} KB)", data=f.read(), file_name=file_name, mime=file_type, key=f"{key_prefix}_download_{notice_id}_{file_name}")
            except Exception:
                pass
    else:
...
LANG = {
    ...
    "en": {
        ...
        "schedule_list": "Registered Schedule List",
    ...
    "hi": {
        ...
        "probability": "संभावना (%)",
        "schedule_list": "पंजीकृत अनुसूची सूची",
        "delete_all": "सभी हटाएं",
        "password_change": "पासवर्ड बदलें",
        "current_password": "वर्तमान पासवर्ड",
        "new_password": "नया पासवर्ड",
        "password_changed_success": "पासवर्ड सफलतापूर्वक बदला गया!",
...
defaults = {"admin": False, "lang": "ko", "notice_open": False, "map_open": False,
            "logged_in_user": None, "show_login_form": False, "show_notice_delete_conf": False, "show_schedule_delete_conf": False}
...
# 데이터 로드
# ==========================================================
tour_notices = load_json(NOTICE_FILE) or []
tour_schedule = load_json(CITY_FILE) or []
user_posts = load_json(USER_POST_FILE) or []

# ==========================================================
# ADMIN_PASS 파일 기반 로드/초기화
# ==========================================================
ADMIN_PASS_DEFAULT = "0691"
...
# 초기 tour_schedule이 비어있다면 city_dict를 기반으로 초기화
...
with col_lang:
    ...
    selected_lang_display = st.selectbox(_("menu"), options=lang_display_names, index=current_lang_index, key="lang_select")
    ...
    if st.session_state.admin:
        ...
        if st.session_state.get("show_notice_delete_conf"):
            with st.form("notice_delete_confirmation_form", clear_on_submit=True):
                ...
                if col_y.form_submit_button(_("delete_all")):
        ...
        with st.expander(_("register"), expanded=False):
            with st.form("notice_form", clear_on_submit=True):
                ...
                uploaded_files = st.file_uploader(_("file_attachment"), type=["png","jpg","jpeg","pdf","txt","zip"], accept_multiple_files=True, key="notice_file_uploader")
        ...
        for notice in notices_to_display:
            ...
            with st.expander(header_text, expanded=False):
                ...
                with col_del:
                    if st.button(_("remove"), key=f"del_n_{notice_id}"):
                        for file_info in notice.get('files', []):
                            if os.path.exists(file_info.get('path','')):
                                try:
                                    os.remove(file_info['path'])
...
with tab2:
            ...
            with st.form("schedule_form", clear_on_submit=True):
                ...
                if submitted:
                    if not city_name_input or not venue_name or not schedule_date:
                        ...
                        if is_duplicate:
                            st.warning("이미 동일한 도시/날짜가 등록되어 있습니다.")
                        else:
                ...
                with st.expander(header_text, expanded=False):
                    ...
                    if st.session_state.get(f"edit_mode_{item_id}"):
                        with st.form(f"edit_form_{item_id}"):
                            ...
                            updated_city = col_uc.selectbox(_("city"), city_options, index=city_options.index(item.get('city') if item.get('city') in city_options else city_options[0]))
    ...
    for item in schedule_for_map:
        lat = item['lat']; lon = item['lon']; date_str = item['date']
        try:
        ...
        marker_icon_html = f"""
            <div style="transform:scale(0.666); opacity:{opacity_val}; text-align:center; white-space:nowrap;">
    ...
    if len(locations) > 1:
        ...
        for i, item in enumerate(schedule_for_map):
            try:
                ...
                continue
        if current_index == -1:
            past_segments = locations; future_segments = []
        elif current_index == 0:
            past_segments = []; future_segments = locations
        else:
            past_segments = locations[:current_index+1]; future_segments = locations[current_index:]
        ...
        if len(future_segments) > 1:
            ...
            for i in range(len(future_segments)-1):
                ...
                folium.PolyLine(locations=[p1,p2], color="transparent", weight=15, opacity=0, tooltip=folium.Tooltip(segment_info, permanent=False, direction="top", sticky=True)).add_to(m)
    elif locations:
        try:
        ...
        folium.Circle(location=locations[0], radius=1000, color='#BB3333', fill=True, fill_color='#BB3333', fill_opacity=0.25 if single_is_past else 0.8, tooltip=_("single_location")).add_to(m)
...
.main-title {{ font-size: 3em; margin-bottom:0.5em; text-shadow:0 0 10px rgba(255,255,255,0.2); }}
.notice-content-box {{ border-left:5px solid var(--accent-gold); background-color: rgba(255,215,0,0.05); padding:10px; border-radius:5px; margin-top:10px; margin-bottom:10px; color:#FAFAFA; }}
.stButton>button {{ background-color:var(--accent-red); color:white; border-radius:8px; border:1px solid var(--accent-red); padding:8px 16px; }}
